...
if (cardSection) {
  cardsObj.map(
    (card) => (cardSection.innerHTML += `<div class="col-6"><div class="card spekrCard"> <div class="spekrImg"> <img src="${card.image1}" alt="chess" class="card-img-left spekrImg1" /> <img src="${card.image2}" alt="Speaker's Image" class="card-img-left spekrImg2" /> </div> <div class="card-body"> <div class="card-title spekrTitle">${card.name}</div> <p class="card-text spekrHeading"> ${card.headline} </p> <p class="card-text text-black spekrText"> ${card.desciption} </p> </div> </div> </div>`),
  );
}
